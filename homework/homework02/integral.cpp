...
double refine(double (*func)(double), double fa, double fb, double a, double b,
              double err = 1e-4) {
  double c = (a + b) / 2;
  double fc = func(c);
  double integral = 0;
  if (abs(2.0 * fc - fa - fb) < err) {
    ...
    integral =
        refine(func, fa, fc, a, c, err) + refine(func, fc, fb, c, b, err);
  }
  return integral;
}

double self_adaptive_integral(double (*func)(double), double a, double b, int n,
                              double err = 1e-4) {
  double integral = 0;
  double dx = (b - a) / n;
  double f0 = func(a);
...
double simpson_integral(double (*func)(double), double a, double b, int n) {
  ...
  double integral = 0;
  for (int i = 0; i < n; i++) {
    integral += fractor * (func(a + i * dx) + 4.0 * func(a + (0.5 + i) * dx) +
                           func(a + (i + 1) * dx));
  }
  return integral;
}

double richardson_integral(double (*func)(double), double a, double b, int n,
                           int k) {
...
double chebyshev_anomaly_integral(double (*func)(double), double a, double b,
                                  int n) {
  double integral = 0;
  double x_i = 0;
  for (int i = 1; i < n + 1; i++) {
